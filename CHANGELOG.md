...
- **EUD-169 — `PATCH /admin/v1/credential-catalog`, a point adjustment of delivery modes (AC-11)**: alongside the existing `PUT` (full replace), a tenant administrator can now update the delivery modes of one or more already-enabled credential configuration ids without resending the whole `enabledConfigurationIds` set. Structurally additive-only — the underlying write is a plain `UPDATE ... WHERE enabled = true`, never an `UPSERT`, so this endpoint can never enable a type as a side effect (EC-10). Declaring an id that exists in the registry but is not currently enabled for the tenant rejects the entire request atomically with `409 credential_configuration_not_enabled` — nothing is written, not even for ids that were enabled (ES-10). A mode outside that type's schema ceiling is rejected the same way the `PUT` already does, `409 delivery_mode_not_eligible` (AC-04). Audited under the same `tenant.credential_catalog.changed` event as the `PUT`, now discriminated by a new `action` field (`replace_catalog` | `patch_delivery_modes`) so a single audit filter answers "who changed this tenant's delivery-mode policy" regardless of which endpoint was used.
- **EUD-169 — `GET /admin/v1/credential-catalog` opened to the tenant's operator (`LEAR`) role**: the operator, who is the actor that ultimately issues a credential, can now read the catalog (eligible delivery modes and schema ceiling per type) to know its options before attempting an issuance, instead of discovering them only against an issuance rejection. `PUT`/`PATCH` remain administrator-only — the write authorization check no longer delegates to the read check, so this change cannot silently propagate to the write path.
